...
p. Natürlich ist die Empfehlung, mit den Hardwarekomponenten nicht sparsam zu sein. Beispielsweise wird der Preisunterschied von SSDs zu 15krpm SAS Platten immer geringer. Statt 4 x 15krpm Platten in einem RAID 10 einzusetzen, könnte man auch 2 SSD Platten in einem RAID 1 einsetzen.
...
pre. 1.000 Hosts * 15 Leseaktionen / 60 Sekunden Intervall
    = ca. 250 Leseaktionen pro Sekunde

pre. 1.000 Hosts * 4 Schreibaktionen / 60 Sekunden Intervall
    = ca. 67 Schreibaktionen pro Sekunde

pre. 10.000 Services * 5 Leseaktionen / 60 Sekunden Intervall
    = ca. 833 Leseaktionen pro Sekunde

pre. 10.000 Services * 2 Schreibaktionen / 60 Sekunden Intervall
    = ca. 333 Inserts/Updates pro Sekunde

p. Bei 1.000 Hosts mit je 10 Services sind das in Summe:

pre. 1083 Leseaktionen pro Sekunde
...
pre. 1 Service * 1440 Minuten (1 Tag) * 30 Tage (1 Monat)
    = ca. 43.200 Elasticsearch Dokumente pro Monat

p. Angenommen, Sie möchten 1.000 Hosts mit je 10 Services überwachen:

pre. 1.000 Hosts * 10 Services * 1440 Minuten (1 Tag) * 30 Tage (1 Monat)
...
p. Die WebGUI selbst ist sehr schnell und kann auf einer Single-Core Maschine mit einem 2GHz Prozssor mehrere tausend HTTP-Abfragen verarbeiten. Der Bottleneck hier kann also entweder nur die Datenabank oder Elasticsearch sein und auch der zur Verfügung stehende Hauptspeicher.
...
p. Bitte beachten Sie, dass der Einsatz von Bloonix auf virtuellen Servern nur für sehr kleine Umgebungen sinnvoll ist.

h2. Alle Angaben ohne Gewähr

p. Alle Berechnungen und Angaben sind ohne Gewähr und können variieren. Sollten Sie ein Beratungsgespräch wünschen, so nehmen Sie bitte Kontakt mit dem Support von Bloonix auf.
