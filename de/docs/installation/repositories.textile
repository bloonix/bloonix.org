...
pre. wget https://download.bloonix.de/repos/debian/bloonix.gpg -O- | apt-key add -

p. Jetzt muss nur noch das passende Bloonix Repository in __/etc/apt/sources.list__ eingetragen werden:

pre. Debian 6: echo "deb https://download.bloonix.de/repos/debian/ squeeze main" >>/etc/apt/sources.list
Debian 7: echo "deb https://download.bloonix.de/repos/debian/ wheezy main" >>/etc/apt/sources.list
Debian 8: echo "deb https://download.bloonix.de/repos/debian/ jessie main" >>/etc/apt/sources.list

pre. Ubuntu 12.04: echo "deb https://download.bloonix.de/repos/ubuntu/ precise main" >>/etc/apt/sources.list
Ubuntu 14.04: echo "deb https://download.bloonix.de/repos/ubuntu/ trusty main" >>/etc/apt/sources.list

p. Falls Ubuntu zum Einsatz kommt, sollte das passende Debian Release zum Ubuntu Release eingetragen werden.

p. Auflisting der Bloonix Pakete:
...
pre. RHEL 6: rpm -ivh https://download.bloonix.de/repos/centos/6/noarch/bloonix-release-0.5-1.el6.noarch.rpm
RHEL 7: rpm -ivh https://download.bloonix.de/repos/centos/7/noarch/bloonix-release-0.5-1.el7.centos.noarch.rpm

p. Auflisting der Bloonix Pakete:

pre. yum search bloonix

h3. Manuelle Installation des Repositories

p. Das Bloonix Repository in __/etc/yum.repos.d/Bloonix.repo__ eintragen:

pre. cat <<EOT >/etc/yum.repos.d/Bloonix.repo
[bloonix]
...
pre. wget https://download.bloonix.de/repos/centos/RPM-GPG-KEY-Bloonix --output-document /etc/pki/rpm-gpg/RPM-GPG-KEY-Bloonix
...
pre. rpm -ivh https://download.bloonix.de/repos/suse/13/noarch/bloonix-release-0.4-1.noarch.rpm

p. Auflisting der Bloonix Pakete:

pre. zypper search bloonix

p. Die Installation der Pakete erfolgt später.
