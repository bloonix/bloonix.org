...
p. The *Bloonix-Server* is the core of the monitoring frameworks and the interface for the Bloonix agents. The Bloonix server sends on request of an agent the configuration of the services and accepts the status and also statistical data of the Bloonix agents. The data will be validated and stored into the database. The Bloonix server also checks whether an event must be released to send a e-mail to an administrator to inform him about the fact that a service is overloaded or is not available any more.

h2. Bloonix-Service-Checker

p. The *Bloonix-Service-Checker* is a part of the Bloonix server and monitors the actuality of the status of Hosts and service. If, for example, the CPU utilisation of a host was not checked since several minutes any more, this can be due to the fact that a Bloonix agent has fallen out. In this case the service-checker would step in as a pseudo agent and deliver a critical status for the service.
...
p. PostgreSQL is used to store the configuration and the data by hosts, services, users and groups. Other database systems are not supported. It is not planned in the future to support MySQL, because the application of Bloonix uses sequences. Sequences are not supported by MySQL.
