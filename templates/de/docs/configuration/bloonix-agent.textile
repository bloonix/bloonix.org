...
p. Möchten Sie zum Beispiel ein Software-RAID oder die Temparatur überwachen, so benötigen die Checks __check-mdadm__ und __check-sensors__ root-Rechte. Damit der Agent diese zwei Checks mit sudo ausführt, muss folgende Konfiguration vorgenommen werden:
...
Defaults:bloonix env_keep += "PLUGIN_LIBDIR CONFIG_PATH CHECK_HOST_ID CHECK_SERVICE_ID"
Defaults:bloonix secure_path="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"

p. Falls Sie Nagios-Plugins einsetzen und diese via __sudo__ ausgeführt werden müssen, dann muss der Nagios-Check in __sudoers__ eingetragen werden. Wenn in der WebGUI ein Nagios-Plugin eingerichtet wird, so kann das Kommando __check-simple-wrapper__ in der Beschreibung des Plugins Verwirrung stiften. Bitte richten Sie keine sudo-Rechte für das Skript __check-simple-wrapper__ ein.
