...
| bloonix-count-distinct-es-service-data | bloonix-server | Anzahl der Dokumente eines Hosts in Elasticsearch. |
...
p. Mit diesem Skript können Host Daten aus Elasticsearch entfernt werden. Die Aufbewahrungsdauer der Daten wird bei der Konfiguration der Hosts in der WebGUI bestimmt.
