...
h2. Remote Checks über einen Agenten oder Satellite

p. Wie in dem Dokument "Wie funktioniert Bloonix":/de/docs/about/how-does-bloonix-work.html beschrieben ist, ist es möglich, einen Agenten/Satellite so zu konfigurieren, dass dieser multiple Hosts von einem beliebigen Standort überachen kann. Beim Einsatz eines Agenten/Satellite, der Remote Checks ausführt, sollte folgendes beaachtet werden, wenn andere Menschen ausser den Administratoren von Bloonix Checks einrichten können:

* Der Agent/Satellite führt Checks aus, die in der WebGUI eingerichtet sind
* Der Agent/Satellite kann Checks über TCP, UDP, HTTP, SMTP, IMAP, SNMP etc. ausführen
...
p. Aus diesen Gründen sollte ein Agent/Satellite, der remote Checks ausführt, immer auf einem Server laufen, der nicht in Ihrem Netzwerk steht und auf dem auch sonst keine anderen Dienste laufen, als der Agent/Satellite selbst.
