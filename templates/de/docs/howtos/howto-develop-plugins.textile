...
p. Zu jedem Plugin gibt es eine spezielle Datei, in der Metadaten abgelegt werden. Schauen wir uns beispielhaft das Plugin __check-http__ an:
...
h2. Regeln, die beachtet werden sollten
...
* Ein Plugin sollte niemals blind alle Parameter, die über die WebGUI konfiguriert werden, annehmen und ausführen.
* Sollte ein Plugin eine spezielle Konfiguration benötigen, so ist diese in /etc/bloonix/agent abzulegen.
