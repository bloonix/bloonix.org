...
h2. Konfiguration des Bloonix-Satellite

p. Nach der Installation muss in der Konfigurationsdatei des Satellite der Parameter __authkey__ angepasst werden. Dieser Key wird zur Authentifizierung verwendet, sprich, damit der Bloonix-Agent über einen Satelliten Checks ausführen darf, muss sich der Agent mit einem Authkey identifizieren. Die Konfigurationsdatei des Satelliten lautet:
...
p. Zusätzlich können Sie SSL Parameter anpassen. Nach der Installation des Satelliten wird ein SSL Zertifikat generiert. Wenn Sie dieses durch ein gültiges Zeritifkat erstezen möchten, so können Sie die Anapassungen hierfür in der Sektion __tcp_server__ vornehmen. Die Sektion sollte Ihnen bereits bekannt sein, da diese identisch mit der Konfigration des Bloonix-Server ist.
...
h2. Einrichtung der Satellites in der WebGUI
...
!{width: 100%; margin: 20px 0;}/public/img/bloonix-location-configuration.png!

!{width: 100%; margin: 20px 0;}/public/img/bloonix-locations.png!
...
!{width: 100%; margin: 20px 0;}/public/img/bloonix-configure-location-check.png!
