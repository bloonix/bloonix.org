...
p. Die Kommunikation zwischen dem Server und den Agenten wird via SSL verschlüsselt. Hierzu wird selbstverständlich ein SSL-Schlüsselpaar für den Server benötigt, welches nach der initialen Installation des Bloonix-Servers generiert und unter __/etc/bloonix/server/pki__ abgelegt wird.

p. Wenn Sie über ein gültiges SSL-Zertifikat verfügen, dann sollten Sie dieses auch verwenden, da Sie ansonsten die Bloonix-Agenten so konfigurieren müssen, so dass diese nicht das Zertifikat des Servers validieren.
